...
export default function Menu() {
  ...
  return (
    ...
    md:top-12 right-6 md:right-32 rounded-md p-4 space-y-4">
              ...
              {user && <h3 className="text-white text-sm hover:text-red-500 cursor-pointer">
                    <Link to="/create">Create</Link></h3>}
              {user && <h3 className="text-white text-sm hover:text-red-500 cursor-pointer">
                    <Link to={"/myBlogs/"+user._id}>My blogs</Link></h3>}
              {user && <h3 onClick={handleLogout} className="text-white text-sm hover:text-red-500 cursor-pointer">Logout</h3>}
